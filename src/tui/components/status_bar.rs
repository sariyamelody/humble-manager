...
impl<'a> Widget for StatusBar<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let mode_span = match self.state.mode {
            ...
            Mode::SyncPrompt | Mode::GenrePicker | Mode::SortPicker | Mode::ColumnPicker => Span::styled(" NORMAL ", Style::default().fg(Color::Black).bg(Color::Green).add_modifier(Modifier::BOLD)),
        ...
        let hint = Span::styled(
            " j/k:move  /:search  f:status  s:sort  S:sort-pick  t:tags  c:source  C:cols  e:export  r:sync  R:enrich  q:quit",
